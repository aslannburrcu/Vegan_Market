...
namespace Vegan_Market.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<ActionResult> Create([Bind(Include = "product_id,product_name,product_price,cate_no,sub_no,brand_no,description,picture_1,picture_2,picture_3,picture_4,product_num,stock_no,product_discount,barcode_no,discount_text_")] Product product,HttpPostedFileBase[] file)
        {
                       ...
                       // string dosya_adi = "resimyok.jpg";
                        ...
                        if (files.ContentLength > 0)//dosya transfer olduysa
                        {
                            ...
                            if (uzanti.Equals(".jpg") || uzanti.Equals(".png"))
                            {
                                ...
                                String yol = Path.Combine(Server.MapPath("~/images"), dosya_adi);
                                files.SaveAs(yol);
                                product.picture_1 = "/images" + dosya_adi;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "product_id,product_name,product_price,cate_no,sub_no,brand_no,description,picture_1,picture_2,picture_3,picture_4,product_num,stock_no,product_discount,barcode_no,discount_text_")] Product product, HttpPostedFileBase[] file)
        {
            if (ModelState.IsValid)
            {
                if (file != null && file[0] != null)
                {
                    foreach (HttpPostedFileBase files in file)
                    {
                        ...
                        if (files.ContentLength > 0)//dosya transfer olduysa
                        {
                            ...
                            String uzanti = Path.GetExtension(files.FileName);
                            if (uzanti.Equals(".jpg") || uzanti.Equals(".jpeg") || uzanti.Equals(".jfif"))
                            {
                                var dosya_adi = Path.GetFileName(files.FileName);
                                String yol = Path.Combine(Server.MapPath("~/images"), dosya_adi);
                                files.SaveAs(yol);
